...
window.EXIF = function () {
  ...
  function getImageData(img, callback) {
    BinaryAjax(img.src, function (oHTTP) {
      var data = findEXIFinJPEG(oHTTP.binaryResponse);
      img.exifdata = data || {};
      if (callback) {
        callback.call(img);
      }
  ...
  function readTags(file, tiffStart, dirStart, strings, bigEnd) {
    var entryOffset = void 0;
    var tag = void 0;
    var high = void 0;
    var val = file.getShortAt(dirStart, bigEnd);
    var tags = {};
    /** @type {number} */
    high = 0;
    for (; val > high; high++) {
  ...
  function readTagValue(file, entryOffset, tiffStart, dirStart, bigEnd) {
    ...
    var numValues = file.getLongAt(entryOffset + 4, bigEnd);
    var valueOffset = file.getLongAt(entryOffset + 8, bigEnd) + tiffStart;
    switch (type) {
      case 1:
      case 7:
        if (numValues === 1) {
  ...
  function pretty(img) {
    ...
    for (a in data) {
      if (data.hasOwnProperty(a)) {
        optsData += _typeof(data[a]) === 'object' ? data[a] instanceof Number ? a + ' : ' + data[a] + ' [' + data[a].numerator + '/' + data[a].denominator + ']\r\n' : a + ' : [' + data[a].length + ' values]\r\n' : a + ' : ' + data[a] + '\r\n';
      }
    }
    return optsData;
  }
