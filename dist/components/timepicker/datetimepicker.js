  ...
  _createClass(DateTimePicker, [{
    ...
    value: function onChange(val, text, listIndex) {
      ...
      if (listIndex === 0) {
        ...
        // 开始日期
        if (onDay === day[0]) {
          var getHour = DateTimePicker._deleteStrUnit(onHours, format[1]);
          // 判断小时是否在开始范围内
        ...
        // 结束日期
        if (onDay === day[len]) {
          var _getHour = DateTimePicker._deleteStrUnit(onHours, format[1]);
      ...
      if (onDay === day[0] && onHours === setHour[0]) {
        var getMinute = DateTimePicker._deleteStrUnit(onMinutes, format[2]);
        ...
        // 结束日期
      } else if (onDay === day[len] && DateTimePicker._deleteStrUnit(onHours, format[1]) === endDateArr.datelist.h) {
        var _getMinute = DateTimePicker._deleteStrUnit(onMinutes, format[2]);
        // 判断分钟是否在开始范围内
    ...
    value: function setInitState() {
      ...
      var onDataTime = DateTimePicker._getTimeRound(defaultData, scale);
      var startDateTime = DateTimePicker._getTimeRound(startData, scale);
      var endDateTime = DateTimePicker._getTimeRound(endData, scale);
      var len = DateTimePicker._getDayDiff(endDateTime.split(' ')[0], startDateTime.split(' ')[0]);
      var onDataArr = DateTimePicker.setDateFormat(onDataTime, 'yyyy/MM/dd hh:mm');
      var startDateArr = DateTimePicker.setDateFormat(startDateTime, 'yyyy/MM/dd hh:mm');
      var endDateArr = DateTimePicker.setDateFormat(endDateTime, 'yyyy/MM/dd hh:mm');

      // 获取时间范围
      var getHoverOver = DateTimePicker._checkHourOver(onDataArr, startDateArr, endDateArr);
      var startMinutes = 0;
      var endMinutes = 59;
      var nowdate = DateTimePicker._getDataTime();
      var tomorrow = DateTimePicker._tomorrowData();
      var valueFirst = DateTimePicker.setDateFormat(onDataTime, 'yyyy/M/d').fmt;
      var setValueFirst = '' + DateTimePicker.setDateFormat(onDataTime, format[0]).fmt;
